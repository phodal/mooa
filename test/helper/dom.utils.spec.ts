...
test('should be able generate iframe id', () => {
  ...
  expect(tmp.innerHTML).toBe(
    '<iframe frameborder="" width="100%" height="100%" src="http://localhost/assets/iframe.html" id="help_206547"></iframe>'
  )
})
...
test('should be able to hidden iframe when coexist mode', () => {
  ...
  tmp.appendChild(htmlElement)
  expect(tmp.innerHTML).toBe(
    '<iframe frameborder="" width="100%" height="100%" src="http://localhost/assets/iframe.html" id="help_206547" style="display: none;"></iframe>'
  )
})
